...
class AdminHelp extends Command {

    constructor() {
        super("AdminHelp");
        this.aliases     = ["AdminCommand", "AdminCommands", "AdmHelp"];
        this.description = "Admin \"Help\" command. (Leave empty for a list of commands.)";
        this.usage       = "<command ...>";
    }

    findCommand(command) {

        for (let i in global.bot.admin) {

            let cmd = global.bot.admin[i];
    ...
    execute(message) {
        ...
        if (params.length) {
            ...
            return global.bot.client.sendMessage(message.author, [
                "**Available Commands:**",
                "```",
                Object.keys(global.bot.admin).join(", "),
                "```",
